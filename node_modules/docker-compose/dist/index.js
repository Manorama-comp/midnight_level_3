...
const MAX_CAPTURED_OUTPUT_LENGTH = buffer_1.constants.MAX_STRING_LENGTH;
...
const appendBufferedOutput = (output, stream, chunk, maxLength) => {
    if (chunk.length === 0) {
        return;
    }
    ...
    const retainedChunk = stream === 'err'
...
const assertCompleteOutput = (result, command) => {
...
const execCompose = (command, args, options = {}) => new Promise((resolve, reject) => {
    ...
    const maxOutputLength = Number.isFinite(requestedMaxOutputLength)
        ? Math.max(0, Math.min(Math.floor(requestedMaxOutputLength), MAX_CAPTURED_OUTPUT_LENGTH))
